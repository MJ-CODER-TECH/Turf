...
const BookingPage = () => {
  ...
  // ✅ FIX: bookingId ki jagah bookingMeta aata hai ab
  const {
    turf,
    selectedDate,
    selectedSlots = [],
  ...
  useEffect(() => {
    // ✅ FIX: bookingMeta check karo
    if (!bookingMeta || !razorpayOrderId) {
      navigate('/', { replace: true });
    }
  }, [bookingMeta, razorpayOrderId, navigate]);
  ...
  const handlePay = useCallback(async () => {
    ...
    const options = {
      ...
      handler: async (response) => {
        try {
          ...
          // ✅ FIX: bookingMeta confirm ke saath bhejo
          await axios.post(
            `${API_BASE_URL}/bookings/confirm`,
            {
              ...
              bookingMeta,       // ← yahan se booking create hogi
            },
    ...
    rzp.open();
  }, [sdkReady, bookingMeta, razorpayOrderId, total, selectedDate, selectedSlots, turf]);

  if (!bookingMeta || !razorpayOrderId) return null;
  ...
  if (step === 3 && success) return (
    <div className="min-h-screen dark:bg-[#0a1628] bg-gray-50 flex items-center justify-center px-4">
      <div className="dark:bg-[#0d1f3c] bg-white border border-green-500/40 rounded-2xl p-10 max-w-md w-full text-center shadow-sm">
        ...
        <p className="dark:text-green-400 text-green-600 text-xs font-black tracking-[4px] uppercase mb-2">Booking Confirmed 🎉</p>
        ...
        <p className="dark:text-green-400 text-green-600 text-2xl font-extrabold mb-2">₹{success.total} Paid</p>
        <p className="dark:text-slate-500 text-slate-400 text-xs mb-8">Confirmation sent to your email & SMS</p>
  ...
  return (
    <div className="min-h-screen dark:bg-[#0a1628] bg-gray-50 pt-24 px-4 pb-16">
      <div className="max-w-lg mx-auto">
        ...
        <div className="mb-8">
          <span className="dark:text-green-400 text-green-600 text-xs font-black tracking-[3px] uppercase">💳 Complete Payment</span>
          <h1 className="dark:text-white text-gray-900 text-3xl font-extrabold mt-2 mb-2">Confirm Booking</h1>
          ...
          <div className="px-6 py-5 space-y-4">
            ...
            <div className="space-y-2">
              <div className="flex justify-between text-sm">
                <span className="dark:text-slate-400 text-slate-500">Base (₹{pricePerHour} × {selectedSlots.length})</span>
        ...
        >
          {sdkLoading ? 'Loading payment...' : paying ? 'Processing...' : `Pay ₹${total} Securely →`}
        </button>
